...
internal fun BundleCounters.calculateCoverageData(
    ...
    val tests = bundlesByTests.map { (typedTestId, bundle) ->
        val testOverview = byTestOverview[typedTestId] ?: TestOverview.empty
        val typedTest = TypedTest(type = typedTestId.type, details = testOverview.details)
        TestCoverageDto(
            id = typedTestId.id(),
            type = typedTest.type,
            name = typedTest.details, // TODO remove
            coverage = bundle.toCoverDto(tree),
...
private fun Map<String, List<Session>>.bundlesByTests(
    ...
    map { (testType, sessions: List<Session>) ->
        sessions.asSequence().flatten()
            .mapNotNull { execData ->
                execData.testId.typedTestId(testType).takeIf { it !in cache }?.to(execData)
            }
...
private fun Sequence<Session>.testsWithBundle(
    ...
    session.tests.map { it.testId.typedTestId(session.testType) }.asSequence()
