...
internal fun Sequence<ExecClassData>.bundle(
    ...
    contents.forEach { execData ->
        classBytes[execData.name]?.let { classesBytes ->
            runCatching { analyzer.analyzeClass(classesBytes, execData.name) }.onFailure {
                logger.error {
                    "Error while analyzing class ${execData.name}. " +
                        "The number of probes received does not match the number of probes provided "
                }
            }
