...
class JavaCoverageTest : PluginTest() {
    ...
    fun `should filter coverage when auto session has several tests`() {
        runBlocking {
            val (coverageData, context) = runAutoSessionWithTwoTests(passedTest = "test1")
            coverageData.run {
                assertEquals(1, coverage.classCount.total)
    ...
    @Test
    fun `should get 0 coverage when filter has 'and' and default`() {
        runBlocking {
            val (coverageData, context) = runAutoSessionWithTwoTests(passedTest = "test1")
            coverageData.run {
                assertEquals(1, coverage.classCount.total)
    ...
    @Test
    fun `should get 0 coverage when filter has 'and'`() {
        runBlocking {
            val (coverageData, context) = runAutoSessionWithTwoTests(passedTest = "test1")
            coverageData.run {
                assertEquals(1, coverage.classCount.total)
    ...
    @Test
    fun `should get 100 coverage when auto session has several tests and filter has few values OR`() {
        runBlocking {
            val (coverageData, context) = runAutoSessionWithTwoTests(passedTest = "test1")
            coverageData.run {
                assertEquals(1, coverage.classCount.total)
